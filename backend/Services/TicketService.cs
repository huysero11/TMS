...
namespace backend.Services
{
    public class TicketService : ITicketService
    {
        ...
        public async Task<TicketDetailResponseDTO> CancelMyTicketAsync(int userId, int ticketId)
        {
            var ticket = await _context.Tickets
                            .FirstOrDefaultAsync(t => t.Id == ticketId && t.CreatedByUserId == userId);
            if (ticket is null)
            ...
            if (ticket.Status == TicketStatus.Closed)
            {
                throw new InvalidOperationException("Closed ticket cannot be cancelled.");
            }
            ...
            var cancelledTicket = await _context.Tickets
                                    .AsNoTracking()
                                    .Where(t => t.Id == ticket.Id)
                                    .Select(t => new TicketDetailResponseDTO
                                    {
                                    ...
                                    .FirstOrDefaultAsync();
            return cancelledTicket ?? throw new Exception("Ticket not found after cancellation");
        }
    }
}